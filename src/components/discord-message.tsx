...
const getBadgestStyles = (color : BadgeColor ) => {
  switch(color) {
    case "#43b581": 
    ...
    <div className="w-full flex items-start justify-start">
      ...
      <div className="w-full max-w-xl">
        <div className="flex items-center">
          ...
          <span className="ml-2 px-1.5 py-0.5 text-xs font-semibold bg-fuchsia-600 text-white rounded">
            APP
          </span>
        ...
        <div className="bg-[#2f3136] text-sm w-full rounded p-3 mb-4 mt-1.5">
          <div className="flex flex-row items-center justify-between mb-2">
            {badgeText ? (
              <span className={cn("inline-flex order-2 items-center rounded-md px-2 py-1 text-xs font-medium ring-1 ring-inset",getBadgestStyles(badgeColor))}>
                  {badgeText}
              </span>
            ) : null}
            ...
            {Object.entries(content).map(([key,value]) => (
              <p key={key} className="text-[#dcddde] text-sm/6"> 
                <span className="text-[#b9bbbe] mr-2">
                  {key}:
                </span>
                  {value}
              </p>
